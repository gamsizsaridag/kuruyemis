...
@section('content')
    ...
    <form role="form" id="form" action="{{route('yonetim.category.update',$category->id)}}" method="post" enctype="multipart/form-data">
        @csrf

        <div class="form-group row">
            <label for="example-text-input" class="col-2 col-form-label">Kategori Adı*</label>
            <div class="col-5">
                <input class="form-control" name="name" required="" value="{{$category->name}}" type="text" >
        ...
        <div class="form-group row">
            ...
            <div class="col-5">
                <input class="form-control" name="must" placeholder="1"  value="{{$category->must}}" type="number" >
            </div>
        </div>
